...
flow, see also [the `httpAuth` service](/backend-system/core-services/http-auth). If you want to extract
more details about authenticated users such as their ownership entity refs, use
[the `userInfo` service](/backend-system/core-services/user-info).
...
properly carried along with the request chain. See [the `httpAuth` service docs](/backend-system/core-services/http-auth)
for more details.

The [service to service auth docs](/auth/service-to-service-auth)
...
rather use [`httpAuth.credentials`](/backend-system/core-services/http-auth) instead as part of their
...
For configuring service-to-service access methods, see [the auth docs](/auth/service-to-service-auth).
