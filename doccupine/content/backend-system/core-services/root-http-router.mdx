...
The `/api/:pluginId/` path prefix is reserved for use by plugins to register their own routes via the [HttpRouter](/backend-system/core-services/http-router) service.
...
Please refer to the [HTTP Router documentation](/backend-system/core-services/http-router#rate-limiting).
