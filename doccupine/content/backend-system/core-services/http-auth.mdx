...
also [the `auth` service](/backend-system/core-services/auth). If you want to extract more details about
authenticated users such as their ownership entity refs, use [the `userInfo` service](/backend-system/core-services/user-info).
...
[the `httpRouter` service](/backend-system/core-services/http-router) when you register your routes.

</Callout>

If you want to further work with the credentials object, [the `auth` service](/backend-system/core-services/auth)
...
have to do to accept limited user access is to inform [the `httpRouter` service](/backend-system/core-services/http-router)
...
have set up using [the `httpRouter` service](/backend-system/core-services/http-router) for your routes,
if any.
