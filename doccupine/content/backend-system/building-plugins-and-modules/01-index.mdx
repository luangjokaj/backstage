...
This section covers how to build your own backend [plugins](/backend-system/architecture/04-plugins) and
[modules](/backend-system/architecture/06-modules). They are sometimes collectively referred to as
backend _features_, and are the building blocks that adopters add to their
[backends](/backend-system/architecture/02-backends).

## Creating a new Plugin

This guide assumes that you already have a Backend project set up. Even if you only want to develop a single plugin for publishing, we still recommend that you do so in a standard Backstage monorepo project, as you often end up needing multiple packages. For instructions on how to set up a new project, see our [getting started](/getting-started#prerequisites) documentation.
...
See [the article on naming patterns](/backend-system/architecture/08-naming-patterns) for
details on how to best choose names/IDs for plugins and related backend system
items.

## Modules

Backend modules are used to extend [plugins](/backend-system/architecture/04-plugins) or other modules with
...
Modules interact with their target plugin or module using the [extension points](/backend-system/architecture/05-extension-points) registered by the plugin, while also being
able to depend on the [services](/backend-system/architecture/03-services) of the target plugin.
...
the HTTP service, see [core services](/backend-system/core-services/01-index). When
...
described in detail with code examples in [the extension points architecture article](/backend-system/architecture/05-extension-points), while the following is a more
...
Before adding custom configuration options, make sure to read [the configuration docs](/conf), in particular the section on [defining configuration for your own plugins](/conf/defining) which explains how to establish a
