...
collective term for backend [plugins](/backend-system/architecture/04-plugins) and
[modules](/backend-system/architecture/06-modules)), that are the subjects of the test.
...
register routes with [the HTTP router service API](/backend-system/core-services/01-index).
