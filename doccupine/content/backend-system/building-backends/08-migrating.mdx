...
package as a result of these steps, and then being able to [migrate plugins](/backend-system/building-plugins-and-modules/08-migrating) as a separate
endeavour later.
...
references to actual [backend system services](/backend-system/architecture/03-services).
...
your service. Please see [the services article](/backend-system/architecture/03-services)
...
For plugins that are private and your own, you can follow a [dedicated migration guide](/backend-system/building-plugins-and-modules/08-migrating) as you see fit, at a
later time.
...
You will use the [extension points](/backend-system/architecture/05-extension-points)
mechanism to extend or tweak the functionality of the plugin. To do that,
you'll make your own bespoke [module](/backend-system/architecture/06-modules) which
