...
Backend modules are used to extend [plugins](/backend-system/architecture/04-plugins) or sometimes other modules with additional features or change existing behavior. They must always be installed in the same backend instance as the plugin that they extend, and may only extend a single plugin. Modules interact with their target plugin using the [extension points](/backend-system/architecture/05-extension-points) registered by the plugin, while also being able to depend on the [services](/backend-system/architecture/03-services) of that plugin.
