...
The MCP Actions Backend exposes [Actions](/backend-system/core-services/actions) registered with the [Actions Registry](/backend-system/core-services/actions-registry) as MCP tools.
...
For details on filtering actions, see the [filtering actions documentation](/backend-system/core-services/actions#filtering-actions).
...
See the [Action Attributes Reference](/backend-system/core-services/actions-registry#action-attributes-reference) for the full attribute definitions and defaults.
...
- You must be using the [New Frontend System](/frontend-system/architecture/00-index).
...
2. If you use [feature discovery](/frontend-system/architecture/10-app#feature-discovery) the plugin will be added automatically, if you prefer explicit registration, register the plugin as a feature like this:
...
This flag is honored by every plugin that creates spans through the [Tracing Service](/backend-system/core-services/tracing), not just MCP Actions.
...
See the [OpenTelemetry tutorial](/tutorials/setup-opentelemetry) to learn how to make these spans available.
