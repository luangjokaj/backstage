...
[3 options](/plugins/call-existing-api):

1. you can
   [access the API directly](/plugins/call-existing-api#issuing-requests-directly),
1. you can create a [backend plugin](/plugins/backend-plugin) if you are
...
[proxying section](/plugins/proxying).
...
`discoveryApi` and
`fetchApi` directly from your frontend plugin or wrapping your calls in a [Utility API](/api/utility-apis).
...
This section describes the steps to wrap your API client in a [Utility API](/api/utility-apis), which are:
