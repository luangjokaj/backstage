...
In order to use these new services your backend needs to be using the [new backend system](/backend-system/building-backends/01-index). If your backend is running the old system, you will need to [migrate it to the new system first](/backend-system/building-backends/08-migrating).
...
In short, this will allow requests through to plugins in your backend, even if they do not include any credentials. The requests will still be treated as unauthenticated however, which not all plugin endpoints may accept. For more information on the impact of this configuration, see the [auth service documentation](/backend-system/core-services/auth).
...
The goal in this section is to completely remove usages of the existing [identity](/backend-system/core-services/identity) and [token manager](/backend-system/core-services/token-manager) services from inside your plugin, and instead use the new [auth](/backend-system/core-services/auth) and [http auth](/backend-system/core-services/http-auth) services. You plugin may still accept the `identity` and `tokenManager` services as optional dependencies from the plugin environment though, in order to avoid breaking the setup for existing users.
