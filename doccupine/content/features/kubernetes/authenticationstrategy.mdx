...
Backstage includes by default some [Kubernetes Auth Providers](/features/kubernetes/authentication) to ease the authentication process to
...
To add a new AuthStrategy, we need to create a new Pinniped [backend module](/backend-system/building-plugins-and-modules/01-index#modules) to extend the Kubernetes-Backend plugin. The Pinniped module will interact with the Kubernetes-Backend plugin through the [extension points](/backend-system/architecture/05-extension-points) registered by the plugin. The Kubernetes-Backend plugin [registers](https://github.com/backstage/backstage/blob/ebe7afad9d19f279469168ca0d4feceb92c1ad36/plugins/kubernetes-backend/src/plugin.ts#L155) multiple extension points like `kubernetesObjectsProvider`, `kubernetesClusterSupplier`, `kubernetesFetcher`, `kubernetesServiceLocator` and the `kubernetesAuthStrategy`.

Notice that this guide assumes that you already installed the [Kubernetes Plugin](/features/kubernetes/installation).
...
We are going to reuse the `PinnipedStrategy` created on the previous section. So when setting up the [Kubernetes Backend plugin](/features/kubernetes/installation#adding-kubernetes-backend-plugin), you could add a new Strategy:
