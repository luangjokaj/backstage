...
[entity descriptor YAML files](/features/software-catalog/descriptor-format). However, companies that
...
detailed in [The Life of an Entity](/features/software-catalog/life-of-an-entity). Reading that article
...
5. The method translates the foreign data model to the native `Entity` form, as
   ...
   [Well-known Annotations](/features/software-catalog/well-known-annotations#backstageiomanaged-by-location)
...
This approach will also allow you to customize the schedule per environment. You can also [add a schema to your config](/conf/defining).
