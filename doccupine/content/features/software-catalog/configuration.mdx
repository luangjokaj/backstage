...
YAML files with [static configuration](/conf).
...
processor _does however_ need an [integration](/integrations) to
understand how to retrieve a given URL. For the example above, you would need to
configure the [GitHub integration](/integrations/github/locations) to
...
box, such as the GitHub [discovery](/integrations/github/discovery)
processor that scans a GitHub organization for
[entity descriptor files](/features/software-catalog/descriptor-format).

Check the [integrations](/integrations) documentation to see what
...
[External Integrations](/features/software-catalog/external-integrations).
...
[GitHub Discovery](/integrations/github/discovery). To enforce the usage of
...
Deleting an entity by UUID, `DELETE /entities/by-uid/:uid`, is allowed when using this mode. It may be rediscovered as noted in [explicit deletion](/features/software-catalog/life-of-an-entity#explicit-deletion).
...
The [processing loop](/features/software-catalog/life-of-an-entity#processing) is
...
[Stitching](/features/software-catalog/life-of-an-entity#stitching) finalizes entities asynchronously via a worker queue. You can tune the following parameters under `catalog.stitchingStrategy`:
