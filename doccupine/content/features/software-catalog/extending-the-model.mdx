...
The Backstage catalog [entity data model](/features/software-catalog/descriptor-format) is based on the
...
- Each kind may or may not have a _type_. For example, there are several well
  ...
  in the interface.
- Entities may have a number of _[annotations](/features/software-catalog/well-known-annotations)_ on
...
If you add an [`apiVersion`](/features/software-catalog/descriptor-format#apiversion-and-kind-required)
...
A [kind](/features/software-catalog/descriptor-format#apiversion-and-kind-required) is an overarching
...
of [well-known annotations](/features/software-catalog/well-known-annotations), but you are free to add
...
There is a set of [well-known relations](/features/software-catalog/well-known-relations), but you are
...
[entity reference](/features/software-catalog/references), but you can invent your own types. You do not
...
The custom processor should be created as a separate module for the catalog plugin. For information on how to set that up, see the [plugin docs](/plugins/backend-plugin#creating-a-backend-plugin). Use `yarn new` and select `backend-module` instead to create a module. For our case, the module ID will be `foobar` and the plugin ID will be `catalog`.
...
Look through the [legacy documentation](/features/software-catalog/extending-the-model--old).
