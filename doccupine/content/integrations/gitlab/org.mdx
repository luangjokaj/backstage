...
[`Group`](/features/software-catalog/descriptor-format#kind-group)
...
Install and configure `@backstage/plugin-events-backend-module-gitlab` as described in [GitLab Discovery — Install and Configure GitLab Events Module](/integrations/gitlab/discovery#install-and-configure-gitlab-events-module). The [gitlab events module](https://github.com/backstage/backstage/tree/master/plugins/events-backend-module-gitlab/README.md) routes received events from the generic topic `gitlab` to more specific ones based on the event type (for example, `gitlab.user_add_to_group`). These more specific events are what the builtin org events support expects.

### Receiving events in Backstage

Set up how your Backstage instance receives these events using one of the options below:

- [Events Setup using HTTP endpoint](/integrations/gitlab/discovery#events-setup-using-http-endpoint)
- [Events Setup using AWS SQS module](/integrations/gitlab/discovery#events-setup-using-aws-sqs-module)
- [Events Setup using Google Pub/Sub module](/integrations/gitlab/discovery#events-setup-using-google-pubsub-module)
- [Events Setup using Kafka module](/integrations/gitlab/discovery#events-setup-using-kafka-module)
