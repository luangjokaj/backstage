...
This section is part of the legacy plugins documentation. While this page already describes the new backend system patterns, the canonical documentation for building backend plugins has moved to [Building Backend Plugins and Modules](/backend-system/building-plugins-and-modules/01-index).
...
**Note: The route shown here matches the default in the current backend plugin template. If you want a `/health` endpoint for health checks, you can add it to your router yourself.**
...
Backstage provides a bunch of `coreServices` out of box, see the more in depth docs [here](/backend-system/core-services/01-index).
