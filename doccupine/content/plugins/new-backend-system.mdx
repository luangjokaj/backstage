...
This section is part of the legacy plugins documentation. The canonical documentation for the backend system has moved to the [Backend System](/backend-system) section, which includes more detailed and up-to-date guides for [building plugins and modules](/backend-system/building-plugins-and-modules/01-index), [architecture](/backend-system/architecture/01-index), and [core services](/backend-system/core-services/01-index).
...
A detailed explanation of the package architecture can be found in the [Backstage Architecture Overview](/overview/architecture-overview#package-architecture). The most important packages to consider for this system are `backend`, `plugin-<pluginId>-backend`, `plugin-<pluginId>-node`, and `plugin-<pluginId>-backend-module-<moduleId>`.
