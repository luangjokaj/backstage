...
This page describes the structure of a plugin for the **old frontend system**. For the new frontend system, see [Building Frontend Plugins](/frontend-system/building-plugins/01-index). The general folder structure is similar, but the plugin wiring in `plugin.ts` differs significantly.
...
the new [Composability System](/plugins/composability).
