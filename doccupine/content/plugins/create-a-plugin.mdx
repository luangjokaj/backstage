...
This page describes creating plugins for the **old frontend system**. For creating plugins using the new frontend system, see [Building Frontend Plugins](/frontend-system/building-plugins/01-index). For creating backend plugins, see [Building Backend Plugins and Modules](/backend-system/building-plugins-and-modules/01-index).
...
invoking the
[`backstage-cli new --select plugin`](/tooling/cli/03-commands#new))
