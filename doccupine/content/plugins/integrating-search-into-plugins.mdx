...
> Knowing what a [collator](/features/search/concepts#collators) is will help you as you build it out.
...
- You must use the `analytics.captureEvent` method, from the `useAnalytics()` hook (detailed [plugin analytics docs are here](/plugins/analytics)).
