...
`app-config.yaml`. It works the same way as [route bindings in the new frontend system](/frontend-system/architecture/36-routes#binding-external-route-references),
for example:
...
external route references. They work the same way as [default targets in the new frontend system](/frontend-system/architecture/36-routes#default-targets-for-external-route-references),
for example:
