...
This section is part of the legacy plugins documentation. For the new frontend system version, see [Internationalization](/frontend-system/building-plugins/07-internationalization). The i18n APIs (`createTranslationRef`, `useTranslationRef`) work the same way in both the old and new frontend systems.
...
For full command reference, see the [CLI commands documentation](/tooling/cli/03-commands#translations-export).
