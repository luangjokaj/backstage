...
This documentation is written for the old frontend system. If you are on the [new frontend system](/frontend-system) you may want to read [its own article](/auth) instead.
...
on how to configure the backend app, see [Sign-in Identities and Resolvers](/auth/identity-resolver).
...
[Azure EasyAuth](/auth/microsoft/azure-easyauth),
[Cloudflare Access](/auth/cloudflare/provider),
[Google Identity-Aware Proxy](/auth/google/gcp-iap-auth)
and [OAuth2 Proxy](/auth/oauth2-proxy/provider).
...
In case you are using a custom authentication providers, you might need to add a [custom `ScmAuthApi` implementation](/auth#custom-scmauthapi-implementation).
...
through [Utility APIs](/api/utility-apis) for each service provider. These
...
[Add authentication provider](/auth/add-auth-provider) for details on adding a new
