...
Backstage instance is to choose a production ready auth provider. See [the auth overview page](/auth) for a full list of providers and how to install and
configure them.

## Backstage User Identity

A user identity within Backstage is built up from two main pieces of
information: a user [entity reference](/features/software-catalog/references), and a set of ownership
...
You set up builtin sign in resolvers using [your app-config](/conf),
...
[one of the available providers](/auth), you will likely have added a
...
Check out [the naming patterns article](/backend-system/architecture/08-naming-patterns) for what rules
