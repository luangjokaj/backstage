...
This documentation is written for [the new frontend system](/frontend-system). If you are on the old frontend system you may want to read [its own article](/auth/index--old) instead.
...
- [Auth0](/auth/auth0/provider)
- [Atlassian](/auth/atlassian/provider)
- [Azure](/auth/microsoft/provider)
- [Azure Easy Auth](/auth/microsoft/azure-easyauth)
- [Bitbucket](/auth/bitbucket/provider)
- [Bitbucket Server](/auth/bitbucketServer/provider)
- [Cloudflare Access](/auth/cloudflare/provider)
- [GitHub](/auth/github/provider)
- [GitLab](/auth/gitlab/provider)
- [Google](/auth/google/provider)
- [Google IAP](/auth/google/gcp-iap-auth)
- [Okta](/auth/okta/provider)
- [OAuth 2 Custom Proxy](/auth/oauth2-proxy/provider)
- [OneLogin](/auth/onelogin/provider)
- [OpenShift](/auth/openshift/provider)
- [VMware Cloud](/auth/vmware-cloud/provider)
...
Using an authentication provider for sign-in is something you need to configure both in the frontend app as well as the `auth` backend plugin. For information on how to configure the backend app, see [Sign-in Identities and Resolvers](/auth/identity-resolver). The rest of this section will focus on how to configure sign-in for the frontend app.
...
Backstage uses [Passport](http://www.passportjs.org/) under the hood, which has a wide library of authentication strategies for different providers. See [Add authentication provider](/auth/add-auth-provider) for details on adding a new Passport-supported authentication method.
