...
[backend golden path](/golden-path/plugins/backend/002-poking-around), then refresh the
...
spec (see the
[backend golden path](/golden-path/plugins/backend/001-first-steps) for details),
