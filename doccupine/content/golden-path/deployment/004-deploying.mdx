...
- **A container image** built from your repository and pushed to a registry
  ...
  [Building the Docker image](/golden-path/deployment/001-docker) is the artifact that gets
  deployed.
...
- **A reachable PostgreSQL database** that the running instance can connect
  ...
  [Configuring the database](/golden-path/deployment/002-database) for details.
...
[Deploying with Kubernetes](/deployment/k8s), which walks through
...
Docker Compose, and the [deployment overview](/deployment)
...
- **Run multiple replicas** behind your load balancer. Backstage is
  ...
  [Scaling Backstage](/golden-path/deployment/007-scaling).
...
[corporate proxy guide](/tutorials/corporate-proxy).
...
- [Config-first development](/golden-path/deployment/005-config-first)
