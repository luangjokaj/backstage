...
This guide walks through how to set up a PostgreSQL database to host your Backstage data. It assumes you've already have a scaffolded Backstage app from following the [Creating your Backstage App](/getting-started) guide.
...
[Start the Backstage app](/getting-started#creating-and-running-a-backstage-application):
...
First we need to pull down the container image, we'll use Postgres 18, check out the [Postgres Version Policy](/overview/versioning-policy#postgresql-releases) to learn which versions are supported.
...
We recommend you read [Setting up authentication](/getting-started/config/authentication) next.
...
- [Manual Knex Rollback](/tutorials/manual-knex-rollback)
