...
The [Technical Overview](/overview/technical-overview#software-catalog-system-model) provides a description of all of the types of entities displayed in the Catalog.

It should be noted that you can also [create your own kinds of entities](/features/software-catalog/extending-the-model#adding-a-new-kind), if you need to model something in your organization that does not map to one of the existing entity types.
...
- `Type` - all
- `Owner` - Owned
- `Lifecycle` - list of [lifecycle](/features/software-catalog/descriptor-format#speclifecycle-required) values of entities in the Catalog
- `Processing Status` - normal
- `Namespace` - The ID of a [namespace](/features/software-catalog/descriptor-format#namespace-optional) to which the entity belongs

You can change the initial setting for the [Owner](/features/software-catalog/catalog-customization#catalog-filters) and [Kind](/features/software-catalog/catalog-customization#catalog-filters) filters.
...
- `Type` - common types are as follows, but you can [create a new type](/features/software-catalog/extending-the-model#adding-a-new-type-of-an-existing-kind) to meet your organization's needs
...
You can modify the columns associated with each kind of entity, following the instructions in [Customizing columns, actions, and table options](/features/software-catalog/catalog-customization#customizing-columns-actions-and-table-options).
...
- Edit - Edit the `catalog-info.yaml` file that defines the entity. See [Updating a Component](/getting-started/update-a-component)
- Star - Designate the entity as a favorite. You can [filter](/getting-started/filter-catalog) the catalog for starred entities.

[Customizing columns, actions, and table options](/features/software-catalog/catalog-customization#customizing-columns-actions-and-table-options) describes how you can modify the actions that are displayed.
...
- `Relations` - see [Viewing entity relationships](/getting-started/viewing-entity-relationships)
