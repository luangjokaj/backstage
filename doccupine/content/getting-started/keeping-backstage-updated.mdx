...
[versions:bump](/tooling/cli/03-commands#versionsbump).
...
The Backstage CLI respects the standard `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` environment variables when `NODE_USE_ENV_PROXY=1` is set. See the [corporate proxy guide](/tutorials/corporate-proxy) for full details.
...
In some cases you could need to downgrade Backstage instance due to some problem or maybe because you are using a test environment to validate the new version of Backstage. You can check the [Manual Rollback using Knex](/tutorials/manual-knex-rollback) guide to know how to rollback migrations using Knex.
