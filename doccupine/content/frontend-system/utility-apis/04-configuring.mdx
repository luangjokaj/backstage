...
To configure your Utility API extension, first you'll need to know its ID. That ID is formed from the API ref ID; check [the naming patterns docs](/frontend-system/architecture/50-naming-patterns) for details.

Our example work API from [the creating section](/frontend-system/utility-apis/02-creating) would have the ID `api:plugin.example.work`. You configure it and all other extensions under the `app.extensions` section of your app-config.
...
Like with other extension types, you replace Utility APIs with your own custom implementation using [extension overrides](/frontend-system/architecture/25-extension-overrides).
...
In this example the overriding extension is kept minimal, but just like any other extension it can also have `deps`, configurability, and inputs. Check out [the Creating section](/frontend-system/utility-apis/02-creating) for more details about that.
