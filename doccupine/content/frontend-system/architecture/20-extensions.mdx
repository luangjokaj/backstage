...
As mentioned in the [previous section](/frontend-system/architecture/10-app), Backstage apps are built up from a tree of extensions. This section will go into more detail about what extensions are, how to create and use them, and how to create your own extensibility patterns.
...
If a plugin or module also provides an `if` predicate, it is combined with the extension-level predicate using logical `AND`. See the [plugin `if` option](/frontend-system/architecture/15-plugins#if-option) and [frontend modules](/frontend-system/architecture/25-extension-overrides#creating-a-frontend-module) sections for more details.
...
For a full list and explanations of all types of core extension data, see the [core extension data reference](/frontend-system/building-plugins/04-built-in-data-refs).
