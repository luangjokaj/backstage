...
To mock [Utility APIs](/frontend-system/architecture/33-utility-apis) that are used by your component, pass API overrides to `renderInTestApp` using the `apis` option. Mock helpers are available from `@backstage/frontend-test-utils` and plugin-specific test utilities. For a deeper look at the available mock APIs and how to create your own, see [Testing with Utility APIs](/frontend-system/utility-apis/05-testing).
