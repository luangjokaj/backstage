...
The main concept is that routes, components, apis are now extensions. You can use the appropriate [extension blueprints](/frontend-system/architecture/23-extension-blueprints) to migrate all of them to extensions.
...
Pages that were previously created using the `createRoutableExtension` extension function can be migrated to the new Frontend System using the `PageBlueprint` [extension blueprint](/frontend-system/architecture/23-extension-blueprints), exported by `@backstage/frontend-plugin-api`.
...
The equivalent utility to replace components created with `createComponentExtension` depends on the context within which the component is used, typically indicated by the naming pattern of the export. Many of these can be migrated to one of the [existing blueprints](/frontend-system/building-plugins/03-common-extension-blueprints), but in rare cases it may be necessary to use [`createExtension`](/frontend-system/architecture/20-extensions#creating-an-extension) directly.
...
Let's begin with [your `-react` package](/architecture-decisions/adr011-plugin-package-structure). The act of exporting TypeScript interfaces and API refs have not changed from the old system. You can typically keep those as-is. For illustrative purposes, this is an example of an interface and its API ref:
...
Since utility APIs are now complete extensions, you may want to take a bigger look at how they used to be used, and what the new frontend system offers. You may for example consider [adding configurability or inputs](/frontend-system/utility-apis/02-creating) to your API, if that makes sense for your current application.
