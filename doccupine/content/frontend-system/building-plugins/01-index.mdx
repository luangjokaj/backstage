...
This section covers how to build your own frontend [plugins](/frontend-system/architecture/15-plugins) and
[overrides](/frontend-system/architecture/25-extension-overrides). They are sometimes collectively referred to as
frontend _features_, and what you install to build up a Backstage frontend [app](/frontend-system/architecture/10-app).

## Creating a new plugin

This guide assumes that you already have a Backstage project set up. Even if you only want to develop a single plugin for publishing, we still recommend that you do so in a standard Backstage monorepo project, as you often end up needing multiple packages. For instructions on how to set up a new project, see our [getting started](/getting-started) documentation.
...
The plugin ID should be a lowercase dash-separated string, while the plugin instance variable should be the camel case version of the ID with a `Plugin` suffix. For more details on naming patterns within the frontend system, see [the article on naming patterns](/frontend-system/architecture/50-naming-patterns). By sticking to these naming patterns you ensure that users of your plugin more easily recognize the exports and features provided by your plugin.

## Adding extensions

The plugin that we created above is empty, and doesn't provide any actual functionality. To add functionality to a plugin you need to create and provide it with one or more [extensions](/frontend-system/architecture/20-extensions). Let's continue by adding a standalone page to our plugin, with a title and icon that appear in the app sidebar.

To create a new extension you typically use pre-defined [extension blueprints](/frontend-system/architecture/23-extension-blueprints), provided either by the framework itself or by other plugins. In this case we'll use `PageBlueprint` from `@backstage/frontend-plugin-api`. We will also need to [create a route reference](/frontend-system/architecture/36-routes#creating-a-route-reference) to use as a reference for our page, allowing us to dynamically create URLs that link to our page.
...
Another type of extensions that is commonly used are [Utility APIs](/frontend-system/utility-apis/01-index). They can encapsulate shared pieces of functionality of your plugin, for example an API client for a backend service. You can optionally export your Utility API for other plugins to use, or allow integrators to replace the implementation of your Utility API with their own. For details on how to define and provide your own Utility API in your plugin, see the section on [creating Utility APIs](/frontend-system/utility-apis/02-creating).
...
For a more complete list of the different kinds of extensions that you can create for your plugin, see the [extension blueprints](/frontend-system/building-plugins/03-common-extension-blueprints) section.

## Related topics

The following guides cover cross-cutting concerns for building frontend plugins:

- [Internationalization (i18n)](/frontend-system/building-plugins/07-internationalization) — Adding translations to your plugin using `createTranslationRef` and `useTranslationRef`.
- [Plugin Analytics](/frontend-system/building-plugins/08-analytics) — Instrumenting user interactions with the Analytics API using `AnalyticsImplementationBlueprint`.
- [Feature Flags](/frontend-system/building-plugins/09-feature-flags) — Defining and using feature flags via the `featureFlags` option of `createFrontendPlugin`.
