...
The create-app CLI requires Node.js Active LTS Release, see the [prerequisites documentation](/getting-started) for all the details.
...
For advanced bootstrap flows that need access to the app tree before the full app is finalized, see [preparing an app in phases](/frontend-system/architecture/10-app#preparing-an-app-in-phases).
...
Plugins are typically installed by adding them as dependencies of your app package and relying on feature discovery to automatically detect them. For details on how this works, including how to manually install plugins or control which packages are discovered, see [Installing Plugins](/frontend-system/building-apps/05-installing-plugins).

### Configure extensions individually

It is possible to enable, disable and configure extensions individually in the `app-config.yaml` config file. To get familiar with what is available for app extensions personalization, go to the [built-in extensions](/frontend-system/building-apps/03-built-in-extensions) documentation. For plugin customizations, we recommend that you read the instructions in each plugin's README file.
...
You can also pass overrides to the features array, for more details, please read the [extension overrides](/frontend-system/architecture/25-extension-overrides) documentation.
